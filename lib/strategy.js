...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(url, accessToken, function (err, body, res) {
    ...
    try {
      ...
      var profile = { provider: 'scool', email: "test@test.test" };

      profile.id = String(json.user.id);
      profile.username = json.user.first_name + ' ' + json.user.last_name;
      profile.displayName = json.user.first_name + ' ' + json.user.last_name;
      profile.name = { familyName: json.user.last_name, givenName: json.user.first_name };
