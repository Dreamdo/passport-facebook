...
function Strategy(options, verify) {
  options = options || {};
  options.authorizationURL = options.authorizationURL || 'https://auth.hundred.org/oauth/authorize';
  options.tokenURL = options.tokenURL || 'https://auth.hundred.org/oauth/token';
  options.scopeSeparator = options.scopeSeparator || ',';

  OAuth2Strategy.call(this, options, verify);
  this.name = 'scool';
  this._profileURL = options.profileURL || 'https://auth.hundred.org/api/v1/me';
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(url, accessToken, function (err, body, res) {
    ...
    try {
      ...
      var profile = { provider: 'scool' };
      profile.id = String(json.user.id);
      profile.displayName = body;//json.user.first_name + ' ' + json.user.last_name;
      profile.name = body;//{ familyName: json.user.last_name, givenName: json.user.first_name };
      
      //if (json.user.email) { profile.emails = [{ value: json.user.email }]; }
